...
class ASTAnalyzer {
    ...
    async analyzeAll(files) {
        ...
        const nonCodeFiles = files.filter(f => !['.js', '.jsx', '.ts', '.tsx'].includes(f.extension));
                ...
                for (const declarator of node.declarations) {
                    if (declarator.id?.type === 'Identifier') {
                        ...
                        if (init?.type === 'ArrowFunctionExpression' || init?.type === 'FunctionExpression') {
                            ...
                            const isReactWrapper = (callee?.type === 'MemberExpression' &&
    ...
    functionReturnsJSX(funcNode) {
        ...
        const checkForJSX = (node) => {
            ...
            for (const key in node) {
                const child = node[key];
                if (Array.isArray(child)) {
                    if (child.some(checkForJSX))
                        return true;
                }
                else if (child && typeof child === 'object') {
                    if (checkForJSX(child))
                        return true;
                }
            }
            return false;
        ...
        const checkReturns = (node) => {
            ...
            if (node.type === 'ReturnStatement' && node.argument) {
                if (checkForJSX(node.argument))
                    return true;
            }
            for (const key in node) {
                const child = node[key];
                if (Array.isArray(child)) {
                    if (child.some(checkReturns))
                        return true;
                }
                else if (child && typeof child === 'object') {
                    if (checkReturns(child))
                        return true;
                }
            }
            return false;
