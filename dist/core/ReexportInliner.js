...
exports.ReexportInliner = void 0;
 ...
 */
class ReexportInliner {
    constructor(srcPath) {
        this.sourceFilesToDelete = new Set();
     ...
     */
    analyzeFileReexports(sourceFile) {
        const exportDeclarations = sourceFile.getExportDeclarations();
        const reexportDecls = exportDeclarations.filter((decl) => decl.getModuleSpecifierValue() !== undefined);
        const exports = [];
     ...
     */
    extractAllDeclarations(sourceFile) {
        const declarations = [];
        const statements = sourceFile.getStatements();
        for (const stmt of statements) {
            // Skip import statements (we handle those separately)
            if (stmt.getKind() === ts_morph_1.SyntaxKind.ImportDeclaration) {
                continue;
            }
            // Skip export declarations (re-exports from other files)
            if (stmt.getKind() === ts_morph_1.SyntaxKind.ExportDeclaration) {
                continue;
            }
     ...
     */
    extractExportedDeclarations(sourceFile, exportNames) {
        const declarations = [];
...
exports.ReexportInliner = ReexportInliner;
